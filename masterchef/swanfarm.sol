...
// File: contracts/SwanToken.sol

pragma solidity 0.6.12;

// SwanToken with Governance.
contract SwanToken is BEP20 {
    ...
    // Min amount to liquify. (default 10 SWANs)
    uint256 public minAmountToLiquify = 10 ether;
    // The swap router, modifiable. Will be changed to Swan's router when our own AMM release
    IPancakeRouter02 public swanRouter;
    // The trading pair
    address public swanPair;
    // In swap and liquify
    ...
    event SwanRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "SWAN::antiWhale: Transfer amount exceeds the maxTransferAmount");
                require(swapEnabled == true, "SWAN::swap: Cannot transfer at the moment");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the SwanToken contract.
     */
    constructor() public BEP20("SwanCake Token", "SWAN") {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "SWAN::transfer: Burn value invalid");

            // default 100% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "SWAN::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the swan pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = swanRouter.WETH();

        _approve(address(this), address(swanRouter), tokenAmount);

        // make the swap
        swanRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        swanRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from swanRouter when swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "SWAN::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "SWAN::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "SWAN::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
     ...
     * Can only be called by the current operator.
     */
    function updateSwanRouter(address _router) public onlyOperator {
        swanRouter = IPancakeRouter02(_router);
        swanPair = IPancakeV2Factory(swanRouter.factory()).getPair(address(this), swanRouter.WETH());
        require(swanPair != address(0), "SWAN::updateSwanRouter: Invalid pair address.");
        emit SwanRouterUpdated(msg.sender, address(swanRouter), swanPair);
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "SWAN::transferOperator: new operator is the zero address");
...
// SwanFarm is the master of SwanCake Token (SWAN). He can make SWAN and he is a fair guy.
//
...
// contract once $SWAN is sufficiently distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract SwanFarm is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        swanPerBlock = INITIAL_EMISSION_RATE;
    ...
    function pendingSwan(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 swanReward = multiplier.mul(swanPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accSwanPerShare = accSwanPerShare.add(
                swanReward.mul(1e12).div(lpSupply)
            );
        }
        uint256 pending = user.amount.mul(accSwanPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 swanReward =
            multiplier.mul(swanPerBlock).mul(pool.allocPoint).div(
                totalAllocPoint
            );
        swan.mint(devAddr, swanReward.div(10));
        swan.mint(address(this), swanReward);
        pool.accSwanPerShare = pool.accSwanPerShare.add(
            swanReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to SwanCake for SWAN allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);		
		payOrLockupPendingSwan(_pid);
        if (_amount > 0) {
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
            if (address(pool.lpToken) == address(swan)) {
                uint256 transferTax = _amount.mul(swan.transferTaxRate()).div(10000);
    ...
    // Deposit LP tokens to SwanCake for SWAN allocation with referral.
    ...
    // Withdraw LP tokens from SwanCake.
	...
	// Pay or lockup pending SWANs.
    function payOrLockupPendingSwan(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        if (block.number >= startBlockHarvest && block.number <= lastBlockHarvest) {
            if (pending > 0 || user.rewardLockedUp > 0) {        
                ...
                // send rewards
                safeSwanTransfer(msg.sender, totalRewards);
    ...
    function updateEmissionRate(uint256 _swanPerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, swanPerBlock, _swanPerBlock);
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if (referrer != address(0) && referrer != _user && refBonusBP > 0) {
            ...
            swan.mint(referrer, refBonusEarned);
