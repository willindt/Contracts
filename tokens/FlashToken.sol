...
contract FlashToken is ERC20, Ownable {
    ...
    uint256 public sellCooldownperiod = 7 * 24 * 1200;
    uint256 public sellLimitRate = 20;

    mapping (address => uint256) private soldAmount;
    mapping (address => uint256) private firstSellblock;
    ...
    event SellLimitRateUpdated(address indexed operator, uint256 oldSellLimitRate, uint256 newSellLimitRate);
    ...
    function enableCooldown(bool value) external onlyOwner {
        require(value != cooldownEnabled, "FLASH::setTrading: CooldownEnabled is already set.");
    ...
    function setSellLimitRate(uint256 newRate) external onlyOwner{
        require(newRate >= 20, "FLASH::setSellLimitRate: Too low Fees");
        emit SellLimitRateUpdated(msg.sender, sellLimitRate, newRate);
        sellLimitRate = newRate;
    }
    ...
    function updateCooltimeLimit(uint256 day) external onlyOwner {
        sellCooldownperiod = day * 24 * 1200;
        emit CoolDownPeriodUpdated(msg.sender, sellCooldownperiod);
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
        	...
        	if(automatedMarketMakerPairs[to]){
                ...
                if(cooldownEnabled){
                    if(firstSellblock[from].add(sellCooldownperiod) > block.number) {
                        soldAmount[from] = soldAmount[from].add(amount).sub(fees);
                    } else {
                        soldAmount[from] = amount.sub(fees);
                        firstSellblock[from] = block.number;
                    }
                    require(soldAmount[from] <= balanceOf(from).mul(sellLimitRate).div(100), "FLASH: Exceed Max Sell Amount, try later");
                }
        	}
