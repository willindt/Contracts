...
contract NostaToken is IBEP20, Ownable {
    using SafeMath for uint256;

    // General Info Nosta token
    string private _NAME = "Nosta";
    string private _SYMBOL = "NOSTA";
    ...
    address payable public _supplyWalletAddress =
        payable(0x683f5C7a783a6C621094689a9b234Cf22aDCBdd7); // Wallet Supply-team
    address payable public _partnerWalletAddress =
        payable(0xbFc9B7F6352C4f684c93d09e049b006f203DBbF5); // Wallet Partners to receive BUSD rewards of Partners and holders
    address payable public _teamWalletAddress =
        payable(0x580229A61fA58291ee518d54B7e82Df259bB0020); // Wallet Team to receive 1% of transaction fees
    address payable public _algoWalletAddress =
        payable(0x301a2372486c9E70c61E750E7962f876Ec66156F); // Wallet Algo to receive 7% of transcation fees when price impact >= 2%
    ...
    // Partners and holders mappings
    mapping(address => int256) private _partners;
    ...
    /// @notice addPartner: reference a partner by the owner
    function addPartner(address _partner) external onlyOwner {
        require(
            !_isPartner[_partner],
            "Address is already registered as partner"
        );
    ...
    /// @notice registerMember: for member to register with their partner address in parameter
    ...
    /// @notice registerMemberByOwner: owner can register a member in case member has trouble doing it by himself
    function registerMemberByOwner(address _member, address _partner)
        external
        onlyOwner
    {
        require(_isPartner[_partner], "Address is not registered as partner");
    ...
    /// @notice totalMembersForPartner: returns the number of members referenced by a partner address in parameter
    function totalMembersForPartner(address _partner)
        external
        view
        returns (int256)
    {
    ...
    /// @notice isMember: checks if address is member, for internal calls
    function isMember(address _member) private view returns (bool) {
        if (_holders[_member] == address(0)) return false;
        return true;
    }

    /// @notice getPartnerForMember: returns the partner address for a member sent in parameter
    function getPartnerForMember(address _member)
        external
        view
        returns (address)
    {
    ...
    /// @notice send some 1,5% tokens to partner if holder is attached to a patner or send to _partnerWalletAddress
    ...
    /// @notice send some 0,5% tokens to holder
    ...
    /// 2% liquidity pool (1% Nosta 1% BUSD) sent to LP (NOSTA/BUSD)
    /// 2% holders and partners sent to _partnerWalletAddress
    ...
    function swapAndLiquify() public onlyOwner lockSwapping {
        ...
        uint256 dividendTokens = newBalance.mul(reflectionFee).div(100);
        bool success = IBEP20(BUSD).transferFrom(
            address(this),
            _partnerWalletAddress,
            dividendTokens
        );
        if (success)
            emit SwapAndSendDividends(_partnerWalletAddress, dividendTokens);
        ...
        // 7% algo
        uint256 sellTokens = newBalance.sub(dividendTokens).sub(teamTokens);
        success = IBEP20(BUSD).transferFrom(
            address(this),
            _algoWalletAddress,
            sellTokens
